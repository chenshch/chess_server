...
local function launch_slave()
    ...
    function handler.message(fd, msg)
        ...
        -- msg is string
        local msg_name,msg_data, msg_session = protopack.unpack(msg)
        session = msg_session
        if agent then
            skynet.redirect(agent, c.client, "client", fd, msg_name, msg_data)
        else
            skynet.send(watchdog, "lua", "socket", "data", fd, msg_name, msg_data)
        end
    end
    ...
    function CMD_SLAVE.response(name, fd, msg)
    ...
    function CMD_SLAVE.push(name, fd, msg)
